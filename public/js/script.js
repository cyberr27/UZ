document.addEventListener("DOMContentLoaded", () => {
  ...
  const displayPrivateMessage = (data) => {
    ...
    deleteBtn.innerHTML = `<img src="/img/trash.png" alt="Delete" />`; // Убедитесь, что у вас есть изображение корзинки в /public/img/trash.png
    deleteBtn.addEventListener("click", () => {
      showConfirmDeleteModal(data.messageId);
    });
  ...
  const showConfirmDeleteModal = (messageId) => {
    ...
    // Обработчик для кнопки "Так"
    newConfirmYes.addEventListener("click", async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`/api/messages/private/${messageId}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        if (response.ok) {
          const messageDiv = document.querySelector(
            `.private-chat-message[data-message-id="${messageId}"]`
          );
          if (messageDiv) {
            messageDiv.remove();
          }
          ...
          const data = await response.json();
          alert(data.error || "Помилка видалення повідомлення");
        }
      } catch (error) {
        alert("Помилка видалення повідомлення: " + error.message);
      }
    });

    // Обработчик для кнопки "Ні"
    newConfirmNo.addEventListener("click", () => {
      modal.classList.add("hidden");
    });
  };

  // Загрузка приватных сообщений при открытии чата
  const loadPrivateMessages = async () => {
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        data.messages.forEach(displayPrivateMessage);
      } else {
        console.error(
          "Помилка завантаження приватних повідомлень:",
          data.error
        );
      }
    } catch (error) {
  ...
  // Обновляем обработчик кнопки чата для загрузки приватных сообщений
  chatBtn.addEventListener("click", () => {
    profileContainer.classList.add("hidden");
    chatContainer.classList.remove("hidden");
    profileEditContainer.classList.add("hidden");
    ...
    loadPrivateMessages(); // Загружаем приватные сообщения
  });

  // Обновляем WebSocket для обработки приватных сообщений
  ws.onmessage = (event) => {
    const data = JSON.parse(event.data);
    if (data.type === "message") {
      displayMessage(data);
